...
Persisted `Room` properties include `floorPolygonJson`, `wallLayoutJson`, `doorLayoutJson`, `windowLayoutJson`, and `interiorDesignLayoutJson` so the browser editor can restore polygon geometry, structure, and components. Floor-plan and interior-design viewports render a physical one-inch grid with emphasized twelve-inch lines; room edges, polygon anchors, and fixture placement and sizing snap to that grid. Both viewports display architectural `|<-- dimension -->|` length and thickness strings for every rectangular or polygon wall segment. The interior-design canvas uses the same room polygon for its floor, wall edges, openings, fixture clipping, and placement limits, and provides one-inch width/depth room resizing controls. Rectangular placement ratios remain the backward-compatible room bounding box.
